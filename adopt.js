...
function renderOptions(type) {
  ...
  breedFilter.innerHTML = '<option value="">품종</option>' + breeds.map(b => `<option value="${b}">${b}</option>`).join('');
  regionFilter.innerHTML = '<option value="">지역</option>' + regions.map(r => `<option value="${r}">${r}</option>`).join('');
}
...
// ✅ 초기 로딩 시 강아지 버튼에 .active 클래스 강제 추가
document.querySelector('.filter-btn[data-type="dog"]').classList.add("active");
renderOptions("dog");
renderCards("dog");
